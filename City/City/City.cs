...
namespace AgencySimulator.City
{
    ...
    public sealed class City : ICity
    {
        ...
        /// </summary>
        private List<IElement> cityElements;
        ...
        public City(string Name, int Width, int Height)
        {
            ...
            cityElements = new List<IElement>();
            cityName = Name;
        }
        ...
        private bool TryToPlaceElement(int Row, int Col, int RightWidth, int DownDepth)
        {
            if (Row + DownDepth > citySize.Item2 || Col + RightWidth > citySize.Item1)
                return false;
            foreach (IElement el in cityElements)
        ...
        /// </summary>
        /// <param name="Bill">Устанавливаемый биллборд.</param>
        public Tuple<IBillboardDrawer, IBillboard> PlaceBillboard(int Cost, int AgencyId)
        {
            ...
            BillboardDrawer BD = new BillboardDrawer(Billboard, citySize.Item2);
            return Tuple.Create((IBillboardDrawer)BD, (IBillboard)Billboard);
        }
            ...
            for (int i = 0; i < L; i++)
                if (cityElements[i].GetType() == typeof(House))
                    cityMatrixProximity.PlaceCityElement(cityElements[i].GetPosition(),
                        ((House)cityElements[i]).GetElementSize());
                else
